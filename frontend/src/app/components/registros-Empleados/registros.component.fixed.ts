...
export class RegistrosComponent implements OnInit {
  empleado: Empleado & { horarioId?: number | null | undefined } = { nombre: '', identificacion: '', fechaContratacion: '', activo: false, rol: '', huellaDactilar: '', horarioId: null };
  huellas: string[] = [];
  horarios: any[] = [];
  botonesBloqueados: boolean = true;

  constructor(private registroService: RegistroEmpleadosService, private loginService: LoginService) {}

  ngOnInit(): void {
    this.getHuellas();
    this.getHorarios();

    this.loginService.botonesInicio$.subscribe(bloqueado => {
